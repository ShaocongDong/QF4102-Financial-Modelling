function optValue = FSGMAmericanFixedLookbackPut(t, T, S0, sigma, q, runningMin, r, K, N)
 ...
 V = zeros(N+1, 2*N+1);
 for k = (-N):1:(N)
    V(N+jshift, k+kshift) = max((K - Minimum(k+kshift)), 0);
 end
 ...
 for n = (N-1):-1:0 % for every time state
    ...
    for j = 0:1:n % for every price state
        ...
        for k = (-n):1:(n) % for every running Min
            ...
            kfloor = floor(log(Aup / runningMin) / dx); % the calculated floor index
            ...
            kfloor = floor(log(Adown / runningMin) / dx); % the calculated floor index
            kfloor_index = max(kfloor + (n + 1) + 1, 1);
